# Hermes TTS - 给你的马安上嘴巴子

Hermes Agent 原生仅支持 Telegram / Discord 的语音交互，不支持飞书语音消息。
本项目为飞书用户补齐了这块能力：通过免费的 TTS 引擎合成语音，由飞书机器人发送语音消息，同时支持音色选择和语音克隆。
...
将 `voice-tts/` 和 `voice-clone/`（可选）目录复制到你的 agent 的 skills 目录，即可使用。
...
cp -r hermes-tts/voice-clone ~/.hermes/skills/    # 声音克隆（可选）
...
cp -r hermes-tts/voice-clone ~/.openclaw/skills/
...
**3. 方式二：使用 voice-clone skill 自动化克隆**

voice-clone skill 支持从网络搜索公众人物音频或手动上传音频来创建声音模型：
...
# 预处理音频（格式转换、裁剪、降噪）
python scripts/prepare_audio.py --input raw.mp4 --duration 25 --denoise

# 创建声音模型
python scripts/clone_voice.py create --name "雷军" --audio prepared.wav
